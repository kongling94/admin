...
class AppController extends RestUserBaseController
{
    protected $postModel;

    public function __construct(AppModel $postModel)
    {
        parent::__construct();
    ...
    public function index()
    {
        ...
        // $sql = Db::query('SELECT a.id AS id, a.name AS name, av.id AS version_id, av.version_name, av.create_time, av.content, av.file FROM ')
        // $data = Db::name('app')->alias('a')->join('__APP_VERSION__ av', 'a.id = av.app_id')->field('a.id, av.update_time')->order('av.update_time')->group('a.id')->select();
        // dump(Db::name('app')->getLastSql());
        // dump($sql_version);
